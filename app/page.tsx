...
export default function Home() {
  return <div className="site-shell"><a className="skip-link" href="#main-content">Skip to content</a><SiteSidebar active="home" /><MobileNavigation active="home" />
    <main className="page-content" id="main-content">
      ...
      {currentlyReading && <section className="reading-preview ruled" aria-labelledby="reading-preview-heading">{currentlyReading.coverUrl && <Image className="reading-preview-cover" src={currentlyReading.coverUrl} alt={`Cover of ${currentlyReading.title} by ${currentlyReading.author}`} width={625} height={1000} sizes="112px" />}<div><p className="eyebrow">Currently reading</p><h2 id="reading-preview-heading">{currentlyReading.title}</h2><p className="reading-author">by {currentlyReading.author}</p><p>{currentlyReading.note}</p></div><Link className="soft-button" href="/reading">Reading shelf <span aria-hidden="true">→</span></Link></section>}
      <section id="featured-work" className="ruled"><div className="section-title"><h2>Featured work</h2><Link className="soft-button" href="/projects">View work</Link></div><p className="intro">Focused browser tools for clearer buying decisions and better reading.</p><div className="project-grid">{projects.map((project) => <ProjectCard key={project.slug} project={project} featured />)}</div></section>
